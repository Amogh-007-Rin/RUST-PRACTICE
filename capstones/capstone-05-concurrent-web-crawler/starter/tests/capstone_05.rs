use capstone_05_starter::{extract_domain, CrawlConfig, CrawlResult, DomainRateLimiter};
use std::sync::Arc;
...
async fn test_rate_limiter_different_domains_no_blocking() {
    ...
    let elapsed = start.elapsed();
    // Different domains; no delay needed.
    assert!(elapsed.as_millis() < 100);
}

// ---------------------------------------------------------------------------
// Crawler tests (these require the Crawler, which uses MockFetcher)
// ---------------------------------------------------------------------------

use capstone_05_starter::{Crawler, MockFetcher};
...
async fn test_crawl_cancellation_stops_early() {
    ...
    // Cancel after a short time (some tasks get to finish, others don't).
