...
use capstone_04_starter::{merge_stats, parse_line, process_file, process_files, LogLevel, Stats};
...
fn level_from_str_macro_parses_levels() {
    assert_eq!(
        capstone_04_starter::level_from_str!("DEBUG"),
        Some(LogLevel::Debug)
    );
    assert_eq!(
        capstone_04_starter::level_from_str!("INFO"),
        Some(LogLevel::Info)
    );
    assert_eq!(
        capstone_04_starter::level_from_str!("WARN"),
        Some(LogLevel::Warn)
    );
    assert_eq!(
        capstone_04_starter::level_from_str!("ERROR"),
        Some(LogLevel::Error)
    );
    assert_eq!(
        capstone_04_starter::level_from_str!("FATAL"),
        None::<LogLevel>
    );
}
...
fn parse_latency_macro_parses_ms_values() {
    assert_eq!(
        capstone_04_starter::parse_latency!("latency=123ms"),
        Some(123)
    );
    assert_eq!(capstone_04_starter::parse_latency!("latency=0ms"), Some(0));
    assert_eq!(
        capstone_04_starter::parse_latency!("latency=ms"),
        None::<u64>
    );
    assert_eq!(
        capstone_04_starter::parse_latency!("latency=123"),
        None::<u64>
    );
    assert_eq!(
        capstone_04_starter::parse_latency!("latency=abcms"),
        None::<u64>
    );
    assert_eq!(
        capstone_04_starter::parse_latency!("duration=99ms"),
        None::<u64>
    );
}
