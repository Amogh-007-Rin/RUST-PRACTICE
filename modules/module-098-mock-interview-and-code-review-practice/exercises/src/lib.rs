...
//!
//! This module contains a FLAWED implementation of an `OrderProcessor`.
//! The code compiles and is clippy-clean, but contains subtle logic bugs.
//! Your job: review the code, find the bugs, and fix them until the tests pass.
//!
//! The bugs are logic-level errors (off-by-one, incorrect calculations,
//! missing checks), not syntax or clippy violations.
...
impl OrderProcessor {
    ...
    pub fn remove_item(&mut self, order_id: u64, item_name: &str) -> bool {
        if let Some(order) = self.orders.iter_mut().find(|o| o.id == order_id) {
            ...
            // BUG: removes ALL items with that name, not just the first one
            let initial_len = order.items.len();
            order.items.retain(|item| item.name != item_name);
            order.items.len() < initial_len
        } else {
            false
        }
    }
    ...
    /// Returns the number of distinct item types in the order, or `None` if
    /// the order doesn't exist.
    pub fn item_count(&self, order_id: u64) -> Option<usize> {
        // BUG: returns distinct item count, not total quantity
        self.orders
            .iter()
