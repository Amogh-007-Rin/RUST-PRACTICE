...
async fn demonstrate_deadlock_avoidance_works() {
    // With tokio::sync::Mutex, holding a lock across .await should not
    // deadlock (the guard yields via the async-aware mutex).
...
fn blocking_computation_is_blocking() {
    // The synchronous function itself works without async.
    let start = std::time::Instant::now();
    let result = blocking_computation("abc");
    let elapsed = start.elapsed();
    assert_eq!(result, "cba");
    // The artificial delay should be measurable.
