//! Integration tests for Module 095 — rate limiter & URL shortener core.
//!
//! Run with: `cargo test -p module-095-exercises`
//!
//! Note how the tests never sleep: time is injected as `now_ms`, so the
//! clock can jump forward arbitrarily. That is the testability payoff of
//! the design taught in this module.

use module_095_exercises::rate_limiter::{SlidingWindow, TokenBucket};
use module_095_exercises::url_shortener::{
...
fn token_bucket_never_exceeds_capacity() {
    ...
    // 100 intervals of inactivity must still cap at capacity
    assert_eq!(bucket.available(10_000), 3.0);
    // consume everything, then check partial refills
    for _ in 0..3 {
...
fn sliding_window_expires_old_requests() {
    ...
    // at t=1000 the request at t=0 has left the window
...
fn hash_map_storage_round_trip() {
    let mut storage = module_095_exercises::url_shortener::HashMapStorage::new();
...
fn storage_trait_works_with_custom_backend() {
    // The whole point of the trait: the shortener doesn't care *how* codes
    // are stored. Here's a second backend, defined entirely in the test.
    struct VecStorage {
...
fn shortener_generates_working_codes() {
    let mut shortener = Shortener::new(module_095_exercises::url_shortener::HashMapStorage::new());
...
fn shortener_handles_collisions_with_retry() {
    // Tiny alphabet + short minimum length forces counter codes to collide;
    // the generator must retry with the next id instead of erroring.
    let mut shortener = Shortener::with_alphabet_and_min_len(
        module_095_exercises::url_shortener::HashMapStorage::new(),
        "ab",
        2,
    );
    let mut codes = Vec::new();
