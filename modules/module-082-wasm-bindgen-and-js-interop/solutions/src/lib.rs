//! Module 082: `wasm-bindgen` & JS interop — reference solution.
//!
...
pub mod dom {
    ...
    impl Dom {
        ...
        pub fn create_element(&mut self, tag: &str, id: &str) -> Result<(), DomError> {
            if self.get_element(id).is_some() {
                return Err(DomError::ElementExists(id.to_string()));
            }
        ...
        pub fn set_text(&mut self, id: &str, text: &str) -> Result<(), DomError> {
            self.elements
                .iter_mut()
                .find(|e| e.id == id)
                .map(|e| e.text_content = text.to_string())
...
pub fn render_todos(dom: &mut Dom, list: &TodoList) -> Result<(), DomError> {
    if dom.get_element("todo-list").is_none() {
        dom.create_element("ul", "todo-list")?;
    }

    let live_ids: Vec<String> = list
        .items()
        .iter()
        .map(|t| format!("todo-{}", t.id))
        .collect();
    let stale: Vec<String> = dom
        .get_all_ids()
        .into_iter()
        .filter(|id| id.starts_with("todo-") && id != "todo-list" && !live_ids.contains(id))
        .collect();
    for id in stale {
    ...
    for todo in list.items() {
        ...
        if dom.get_element(&id).is_none() {
            dom.create_element("li", &id)?;
        }
        dom.set_text(&id, &todo.text)?;
        dom.set_class(&id, if todo.done { "done" } else { "" })?;
    }
    Ok(())
}
