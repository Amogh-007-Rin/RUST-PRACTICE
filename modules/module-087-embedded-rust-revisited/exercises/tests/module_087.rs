...
fn led_register_masks_high_nibble() {
    let mut device = EmbeddedDevice::new();
    device.write_register(LED_REG, 0xFA); // bits 7-4 should be masked
...
fn other_registers_are_not_masked() {
    let mut device = EmbeddedDevice::new();
    device.write_register(CONFIG_REG, 0xF0); // full byte, no masking
...
fn handle_interrupt_ignores_disabled() {
    ...
    let result = device.handle_interrupt();
    assert_eq!(result, Some(1)); // handles enabled IRQ 1, not disabled IRQ 2
    assert_eq!(
...
fn timer_tick_sets_flag_and_increment_on_handle() {
    ...
    device.timer_tick();
    assert_ne!(
        device.get_pending_interrupts() & (1 << TIMER_IRQ),
        0,
        "timer flag must be set"
    );
    ...
    assert_eq!(
        device.get_pending_interrupts() & (1 << TIMER_IRQ),
        0,
        "flag must be cleared after handling"
    );
}
