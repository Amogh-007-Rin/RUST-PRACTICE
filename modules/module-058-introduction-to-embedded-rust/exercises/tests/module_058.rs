use module_058_exercises::{blink_led, DelayMs, GpioInput, GpioOutput, MockTimer, VirtualPin};
...
fn blink_led_three_times() {
    ...
    // After 3 complete on/off cycles, pin should be low (ended on low)
    assert!(pin.is_low());
    // 3 cycles × (500 on + 500 off) = 3000 ms
